...
@pytest.mark.asyncio
async def test_create_item(item_service: ItemService, sample_item_data: ItemCreate) -> None:
    ...
    """
    item = await item_service.create_item(sample_item_data)
...
@pytest.mark.asyncio
async def test_get_item(item_service: ItemService, sample_item_data: ItemCreate) -> None:
    ...
    retrieved_item = await item_service.get_item(created_item.id)
...
async def test_get_item_not_found(item_service: ItemService) -> None:
    ...
    with pytest.raises(NotFoundError) as exc_info:
        await item_service.get_item("non-existent-id")

    assert "not found" in str(exc_info.value).lower()


@pytest.mark.asyncio
async def test_list_items_empty(item_service: ItemService) -> None:
...
@pytest.mark.asyncio
async def test_list_items_with_pagination(item_service: ItemService) -> None:
...
@pytest.mark.asyncio
async def test_list_items_available_only(item_service: ItemService) -> None:
...
@pytest.mark.asyncio
async def test_update_item(item_service: ItemService, sample_item_data: ItemCreate) -> None:
    ...
    updated_item = await item_service.update_item(created_item.id, update_data)
...
async def test_update_item_not_found(item_service: ItemService) -> None:
    ...
    with pytest.raises(NotFoundError):
        await item_service.update_item("non-existent-id", update_data)


@pytest.mark.asyncio
async def test_delete_item(item_service: ItemService, sample_item_data: ItemCreate) -> None:
    ...
    await item_service.delete_item(created_item.id)

    # Verify the item is deleted
    with pytest.raises(NotFoundError):
        await item_service.get_item(created_item.id)


@pytest.mark.asyncio
async def test_delete_item_not_found(item_service: ItemService) -> None:
    ...
    """
    with pytest.raises(NotFoundError):
        await item_service.delete_item("non-existent-id")


@pytest.mark.asyncio
async def test_get_item_count(item_service: ItemService) -> None:
...
@pytest.mark.asyncio
async def test_get_item_count_available_only(item_service: ItemService) -> None:
    ...
    # Total count
    total_count = await item_service.get_item_count()
    assert total_count == 5

    # Available count (0, 2, 4 = 3 items)
    available_count = await item_service.get_item_count(available_only=True)
